...
class Paginate {
  constructor(request) {
    const options = this._getOptions(request.server);
    const defaultLimit = options.defaultPageLimit;
    const maxLimit = options.maxPageLimit;
  ...
  _getOptions(server) {
    return Object.assign({
      defaultPageLimit: 25,
      maxPageLimit: 200,
    }, server.plugins['hapi-bookshelf-jsonapi'].options.pagination);
  }
