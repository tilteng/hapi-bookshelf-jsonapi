...
module.exports.register = function register(server, registry) {
  ...
  registry.forEach(function (resource) {
    ...
    function callHook(hookName, request, state) {
      var hook;
      try {
        hook = server.methods.jsonapi[resource.type][hookName];
      } catch (err) {}
      if (!hook) return Promise.resolve(state);
      return Promise.resolve()
        .then(function () { return hook.call(request, state); })
        .then(function (ret) { return typeof ret === 'undefined' ? state : ret; });
    }
    ...
    server.route({
      ...
      handler: catchErrors(function (request, reply) {
        ...
        callHook('beforeCreate', request, attrs)
          .then(attrs => new ResModel(attrs).save(null, {method: 'insert'}))
    ...
    server.route({
      ...
      handler: catchErrors(function (request, reply) {
        ...
        const serializer = resource.getSerializer(query);
        const attrs = serializer.deserialize(request.payload)
        callHook('beforeUpdate', request, attrs)
          .then(function (attrs) {
            if (updatedCol)
    ...
    server.route({
      ...
      handler: catchErrors(function (request, reply) {
        callHook('beforeDelete', request, request.params.id)
          .then(function (id) {
            ResModel.where(`${tableName}.id`, id).destroy()
          })
          .then(function () {
