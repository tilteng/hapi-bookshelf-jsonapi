...
const Included = require('./included').Included;
...
module.exports.register = function register(server, registry) {
  ...
  registry.forEach(function (resource) {
    ...
    function serializeResource(model, included) {
    ...
    server.route({
      ...
      handler: function (request, reply) {
        Promise.resolve(request.payload)
          ...
          .then(function (model) {
            return model.refresh({withRelated: loaded});
          })
          .then(function (model) {
    ...
    server.route({
      ...
      handler: function (request, reply) {
        const included = new Included();
        ResModel.where('id', request.params.id).fetch({withRelated: loaded})
          .then(checkMissing)
          .then(function (model) {
            return reply({
    ...
    server.route({
      ...
      handler: function (request, reply) {
        Promise.resolve(request.payload)
          ...
          .then(checkMissing)
          .then(model => model.refresh({withRelated: loaded}))
          .then(function (model) {
            return reply({data: serializer.serialize(model)});
          })
          .catch(err => reply(err));
    ...
    resource.forEachRelMany(function (rel, relInfo) {
      const relLoaded = relInfo.resource.getRelLoaded();
        ...
        handler: function (request, reply) {
          ...
          ResModel.where('id', request.params.id)
            ...
            .then(function (model) {
              ...
              if (relLoaded) return collection.load(relLoaded);
